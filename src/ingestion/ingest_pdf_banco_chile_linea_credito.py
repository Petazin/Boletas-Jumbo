...
def get_source_id(conn, source_name, source_type):
    """
    Obtiene el ID de una fuente. Si no existe, la crea.
    Esta versión maneja el caso donde 'nombre_fuente' es UNIQUE.
    """
...
def main():
    pdf_directory = r'C:\Users\Petazo\Desktop\Boletas Jumbo\fuentes\Banco\Banco de Chile\linea de crédito'
        ...
        for pdf_path in pdf_files:
            ...
            try:
                ...
                try:
                    ...
                    # institution_type = path_parts[fuentes_index + 1]  # e.g., 'Banco'
                    source_institution = path_parts[fuentes_index + 2]  # e.g., 'Banco de Chile'
                except (ValueError, IndexError) as e:
                    logging.error(f"No se pudo determinar la institución desde la ruta del archivo: {pdf_path}. Error: {e}")
                    continue
                
                # Para la tabla 'fuentes', el tipo es el instrumento financiero.
                instrument_type = "Línea de Crédito"

...
def is_file_processed(conn, file_hash):
    ...
    cursor.execute("SELECT 1 FROM raw_metadatos_documentos WHERE file_hash = %s", (file_hash,))
    result = cursor.fetchone() is not None
    return result

def get_source_id(conn, source_name, source_type):
    """Obtiene o inserta una fuente y devuelve su ID."""
    cursor = conn.cursor(buffered=True)
    query = "SELECT fuente_id FROM fuentes WHERE nombre_fuente = %s AND tipo_fuente = %s"
    cursor.execute(query, (source_name, source_type))
    result = cursor.fetchone()
    if result:
        return result[0]
    else:
        insert_query = "INSERT INTO fuentes (nombre_fuente, tipo_fuente) VALUES (%s, %s)"
        cursor.execute(insert_query, (source_name, source_type))
        conn.commit()
        logging.info(f"Nueva fuente creada en la base de datos: {source_name} ({source_type})")
...
def insert_raw_national_cc_to_staging(conn, metadata_id, source_id, raw_df):
    ...
    query = f'''
    INSERT INTO staging_tarjeta_credito_banco_de_chile_nacional (metadata_id, fuente_id, {cols})
...
def main():
    ...
    xls_directory = r'c:\Users\Petazo\Desktop\Boletas Jumbo\fuentes\Banco\Banco de Chile\tarjeta de credito\nacional'
    ...
    with db_connection() as conn:
        ...
        for xls_path in xls_files:
            # --- Identificación dinámica de la fuente ---
            path_parts = os.path.normpath(xls_path).split(os.sep)
            try:
                fuentes_index = path_parts.index('fuentes')
                source_institution = path_parts[fuentes_index + 2]
            except (ValueError, IndexError) as e:
                logging.error(f"No se pudo determinar la institución desde la ruta del archivo: {xls_path}. Error: {e}")
                continue
            
            instrument_type = "Tarjeta de Crédito Nacional"
            source_id = get_source_id(conn, source_institution, instrument_type)
            # --- Fin de la identificación dinámica ---

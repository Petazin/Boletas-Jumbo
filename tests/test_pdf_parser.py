...
def test_process_pdf_success(mock_open, mock_categorize_product):
    ...
    with mock.patch("pdf_parser.pypdf.PdfReader") as MockPdfReaderClass:
        ...
        sku_a = "7801234567890"
        product_a = next(p for p in products_data if p["codigo_SKU"] == sku_a)
        ...
        assert (
            product_a["Cantidad_reducida_del_total"] == 0.0
        )
        assert product_a["Categoria"] == "Categoria Mock"

        # Verificar Producto B con oferta
        sku_b = "7809876543210"
        product_b = next(p for p in products_data if p["codigo_SKU"] == sku_b)
        assert (
            product_b["Descripcion_producto"] == "PRODUCTO B"
        )
        assert (
            product_b["Cantidad_unidades"] == 2
        )
        assert (
            product_b["Valor_Unitario"] == 1000.0
        )
        assert (
            product_b["Total_a_pagar_producto"] == 2000.0
        )

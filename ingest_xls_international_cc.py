...
def process_international_cc_xls_file(xls_path, source_id, metadata_id):
    ...
    try:
        ...
        column_mapping = {
            ...
            'Monto Moneda Origen': 'charges_pesos', # Monto en CLP
            'Monto (USD)': 'amount_usd', # Monto en USD
            'País': 'country'
        }
        ...
        # Manejar la ausencia de 'Cuotas' (installments_raw) en cartolas internacionales.
        if 'installments_raw' in df_transactions.columns:
            ...
            df_transactions['current_installment'] = 1
            df_transactions['total_installments'] = 1
        ...
        # Manejar la ausencia de amount_usd y country en cartolas nacionales
        if 'amount_usd' in df_transactions.columns:
            df_transactions['amount_usd'] = df_transactions['amount_usd'].apply(parse_and_clean_value)
            # Calcular exchange_rate (evitar división por cero)
            df_transactions['exchange_rate'] = df_transactions.apply(
                lambda row: row['charges_pesos'] / row['amount_usd'] if row['amount_usd'] != 0 else 0.0,
                axis=1
            )
        else:
            df_transactions['amount_usd'] = None
            df_transactions['exchange_rate'] = None
        
        if 'country' not in df_transactions.columns:
            df_transactions['country'] = None
